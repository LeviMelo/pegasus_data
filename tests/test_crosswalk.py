...
def test_cnpj_enrichment_is_additive_and_never_multiplies(monkeypatch) -> None:
    monkeypatch.setattr(
        crosswalk,
        "_crosswalk_rows",
        lambda: {"2001578": [("12345678000195", "202001", "", "CADGERBR")]},
    )
...
def test_ambiguous_temporal_crosswalk_returns_null_not_extra_rows(monkeypatch) -> None:
    monkeypatch.setattr(
        crosswalk,
        "_crosswalk_rows",
        lambda: {"1": [("12345678000195", "202001", "", "A"), ("11222333000181", "202001", "", "B")]},
    )
...
def test_competence_selects_historical_relation(monkeypatch) -> None:
    monkeypatch.setattr(
        crosswalk,
        "_crosswalk_rows",
        lambda: {"1": [("12345678000195", "202001", "202012", "A"), ("11222333000181", "202101", "", "A")]},
    )
...
def test_explicit_explode_is_required_to_multiply_rows(monkeypatch) -> None:
    monkeypatch.setattr(
        crosswalk,
        "_crosswalk_rows",
        lambda: {"1": [("12345678000195", "", "", "A"), ("11222333000181", "", "", "B")]},
    )
